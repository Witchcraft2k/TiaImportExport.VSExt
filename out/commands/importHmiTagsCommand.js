...
exports.importHmiTagsCommand = importHmiTagsCommand;
const vscode = __importStar(require("vscode"));
const logger_1 = require("../utils/logger");
const workspace_1 = require("../utils/workspace");
async function importHmiTagsCommand(importService, item) {
    try {
        ...
        if (!item) {
            logger_1.Logger.warn('Import HMI tags failed - no item selected');
        ...
        if (!projectName) {
            logger_1.Logger.warn('Import HMI tags failed - no project selected');
        ...
        if (!deviceId) {
            logger_1.Logger.error('Could not determine device ID for HMI tags');
            vscode.window.showErrorMessage('Import from TiaPortal: Could not determine HMI device.');
            return;
        }
        logger_1.Logger.section(`IMPORT HMI TAGS: ${item.label}`);
        ...
        await vscode.window.withProgress({
            ...
            progress.report({ message: `Importing HMI tags...` });
            logger_1.Logger.startOperation(`Import HMI tags`);
            const result = await importService.importHmiTags(deviceId, exportPath);
            if (result.success) {
                logger_1.Logger.success(`HMI tags imported: ${result.itemCount || 0} items`);
                ...
                logger_1.Logger.endOperation(`Import HMI tags`, true);
                vscode.window.showInformationMessage(`Import from TiaPortal: HMI tags imported (${result.itemCount || 0} items)`);
            }
            else {
                ...
                logger_1.Logger.logImportSummary(result);
                logger_1.Logger.endOperation(`Import HMI tags`, false);
        ...
        logger_1.Logger.error('Failed to import HMI tags', error);
...
//# sourceMappingURL=importHmiTagsCommand.js.map