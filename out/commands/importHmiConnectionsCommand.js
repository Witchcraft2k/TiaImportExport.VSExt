...
exports.importHmiConnectionsCommand = importHmiConnectionsCommand;
const vscode = __importStar(require("vscode"));
const logger_1 = require("../utils/logger");
const workspace_1 = require("../utils/workspace");
async function importHmiConnectionsCommand(importService, item) {
    try {
        ...
        if (!item) {
            logger_1.Logger.warn('Import HMI connections failed - no item selected');
        ...
        if (!projectName) {
            logger_1.Logger.warn('Import HMI connections failed - no project selected');
        ...
        if (!deviceId) {
            logger_1.Logger.error('Could not determine device ID for HMI connections');
            vscode.window.showErrorMessage('Import from TiaPortal: Could not determine HMI device.');
            return;
        }
        logger_1.Logger.section(`IMPORT HMI CONNECTIONS: ${item.label}`);
        ...
        await vscode.window.withProgress({
            ...
            progress.report({ message: `Importing HMI connections...` });
            logger_1.Logger.startOperation(`Import HMI connections`);
            const result = await importService.importHmiConnections(deviceId, exportPath);
            if (result.success) {
                logger_1.Logger.success(`HMI connections imported: ${result.itemCount || 0} items`);
                ...
                logger_1.Logger.endOperation(`Import HMI connections`, true);
                vscode.window.showInformationMessage(`Import from TiaPortal: HMI connections imported (${result.itemCount || 0} items)`);
            }
            else {
                ...
                logger_1.Logger.logImportSummary(result);
                logger_1.Logger.endOperation(`Import HMI connections`, false);
        ...
        logger_1.Logger.error('Failed to import HMI connections', error);
...
//# sourceMappingURL=importHmiConnectionsCommand.js.map