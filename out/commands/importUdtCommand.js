...
exports.importUdtCommand = importUdtCommand;
const vscode = __importStar(require("vscode"));
const logger_1 = require("../utils/logger");
const workspace_1 = require("../utils/workspace");
async function importUdtCommand(importService, item) {
    try {
        ...
        if (!item) {
            logger_1.Logger.warn('Import UDT failed - no item selected');
            vscode.window.showWarningMessage('Import from TiaPortal: No data type selected');
            return;
        }
        // Get project name and import path
        const projectName = importService.getCurrentProjectName();
        if (!projectName) {
            logger_1.Logger.warn('Import UDT failed - no project selected');
        ...
        logger_1.Logger.section(`IMPORT UDT: ${item.label}`);
        ...
        await vscode.window.withProgress({
            ...
            logger_1.Logger.startOperation(`Import UDT: ${item.label}`);
            const result = await importService.importSingleUdt(item.id, item.parentId || '', exportPath);
            if (result.success) {
                logger_1.Logger.success(`UDT imported: ${item.label}`);
                ...
                logger_1.Logger.endOperation(`Import UDT: ${item.label}`, true);
                vscode.window.showInformationMessage(`Import from TiaPortal: Data type "${item.label}" imported`);
            }
            else {
                ...
                logger_1.Logger.logImportSummary(result);
                logger_1.Logger.endOperation(`Import UDT: ${item.label}`, false);
        ...
        logger_1.Logger.error('Failed to import UDT', error);
        vscode.window.showErrorMessage(`Import from TiaPortal: ${message}`);
    }
}
//# sourceMappingURL=importUdtCommand.js.map