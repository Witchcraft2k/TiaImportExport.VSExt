...
exports.importUdtsCommand = importUdtsCommand;
const vscode = __importStar(require("vscode"));
const logger_1 = require("../utils/logger");
const workspace_1 = require("../utils/workspace");
async function importUdtsCommand(importService, item) {
    try {
        ...
        if (!item) {
            logger_1.Logger.warn('Import UDTs failed - no folder selected');
            vscode.window.showWarningMessage('Import from TiaPortal: No data type folder selected');
            return;
        }
        // Get project name and import path
        const projectName = importService.getCurrentProjectName();
        if (!projectName) {
            logger_1.Logger.warn('Import UDTs failed - no project selected');
        ...
        logger_1.Logger.section(`IMPORT USER DATA TYPES`);
        ...
        await vscode.window.withProgress({
            ...
            if (result.success) {
                ...
                if (deletedCount > 0) {
                    logger_1.Logger.success(`UDTs imported: ${deletedCount} deleted, ${result.skippedCount ?? 0} unchanged`);
                }
                else {
                    logger_1.Logger.success(`UDTs imported: ${result.successCount ?? result.itemCount} types`);
                }
                ...
                logger_1.Logger.endOperation(`Import UDTs`, true);
                const countMsg = result.successCount ?? result.itemCount;
                vscode.window.showInformationMessage(`Import from TiaPortal: Data types imported (${countMsg} types)`);
            }
            else {
                ...
                logger_1.Logger.endOperation(`Import UDTs`, false);
        ...
        logger_1.Logger.error('Failed to import UDTs', error);
        vscode.window.showErrorMessage(`Import from TiaPortal: ${message}`);
    }
}
//# sourceMappingURL=importUdtsCommand.js.map