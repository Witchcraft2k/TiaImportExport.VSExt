...
exports.importBlockGroupCommand = importBlockGroupCommand;
const vscode = __importStar(require("vscode"));
const logger_1 = require("../utils/logger");
const workspace_1 = require("../utils/workspace");
async function importBlockGroupCommand(importService, item) {
    try {
        ...
        if (!item) {
            logger_1.Logger.warn('Import block group failed - no folder selected');
            vscode.window.showWarningMessage('Import from TiaPortal: No block folder selected');
            return;
        }
        // Get project name and import path
        const projectName = importService.getCurrentProjectName();
        if (!projectName) {
            logger_1.Logger.warn('Import block group failed - no project selected');
        ...
        logger_1.Logger.section(`IMPORT BLOCK GROUP: ${item.label}`);
        logger_1.Logger.info(`Project: ${projectName}`);
        logger_1.Logger.info(`Group ID: ${item.id}`);
        ...
        await vscode.window.withProgress({
            ...
            progress.report({ message: `Importing block folder: ${item.label}...` });
            logger_1.Logger.startOperation(`Import block group: ${item.label}`);
            const result = await importService.importBlockGroupWithPath(item.id, item.parentId || '', groupName, groupPath, exportPath);
            if (result.success) {
                const deletedCount = result.messages?.filter(m => m.type === 'deleted').length ?? 0;
                const resultWithDeleted = { ...result, deletedCount };
                if (deletedCount > 0) {
                    logger_1.Logger.success(`Block group imported: ${deletedCount} deleted, ${result.skippedCount ?? 0} unchanged`);
                }
                else {
                    logger_1.Logger.success(`Block group imported: ${result.successCount ?? result.itemCount} blocks`);
                }
                ...
                logger_1.Logger.endOperation(`Import block group: ${item.label}`, true);
                const countMsg = result.successCount ?? result.itemCount;
                vscode.window.showInformationMessage(`Import from TiaPortal: Folder "${item.label}" imported (${countMsg} blocks)`);
            }
            else {
                ...
                logger_1.Logger.endOperation(`Import block group: ${item.label}`, false);
...
//# sourceMappingURL=importBlockGroupCommand.js.map