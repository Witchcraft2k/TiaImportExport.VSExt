...
exports.importHmiScreensCommand = importHmiScreensCommand;
const vscode = __importStar(require("vscode"));
const logger_1 = require("../utils/logger");
const workspace_1 = require("../utils/workspace");
async function importHmiScreensCommand(importService, item) {
    try {
        ...
        if (!item) {
            logger_1.Logger.warn('Import HMI screens failed - no item selected');
        ...
        if (!projectName) {
            logger_1.Logger.warn('Import HMI screens failed - no project selected');
        ...
        if (!deviceId) {
            logger_1.Logger.error('Could not determine device ID for HMI screens');
            vscode.window.showErrorMessage('Import from TiaPortal: Could not determine HMI device.');
            return;
        }
        logger_1.Logger.section(`IMPORT HMI SCREENS: ${item.label}`);
        ...
        await vscode.window.withProgress({
            ...
            progress.report({ message: `Importing HMI screens...` });
            logger_1.Logger.startOperation(`Import HMI screens`);
            const result = await importService.importHmiScreens(deviceId, exportPath);
            if (result.success) {
                logger_1.Logger.success(`HMI screens imported: ${result.itemCount || 0} items`);
                ...
                logger_1.Logger.endOperation(`Import HMI screens`, true);
                vscode.window.showInformationMessage(`Import from TiaPortal: HMI screens imported (${result.itemCount || 0} items)`);
            }
            else {
                ...
                logger_1.Logger.logImportSummary(result);
                logger_1.Logger.endOperation(`Import HMI screens`, false);
        ...
        logger_1.Logger.error('Failed to import HMI screens', error);
...
//# sourceMappingURL=importHmiScreensCommand.js.map