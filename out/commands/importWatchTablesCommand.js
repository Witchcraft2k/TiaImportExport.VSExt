...
exports.importWatchTablesCommand = importWatchTablesCommand;
const vscode = __importStar(require("vscode"));
const logger_1 = require("../utils/logger");
const workspace_1 = require("../utils/workspace");
async function importWatchTablesCommand(importService, item) {
    try {
        ...
        if (!item) {
            logger_1.Logger.warn('Import watch tables failed - no folder selected');
            vscode.window.showWarningMessage('Import from TiaPortal: No watch table folder selected');
            return;
        }
        // Get project name and import path
        const projectName = importService.getCurrentProjectName();
        if (!projectName) {
            logger_1.Logger.warn('Import watch tables failed - no project selected');
        ...
        logger_1.Logger.section(`IMPORT WATCH TABLES`);
        ...
        await vscode.window.withProgress({
            ...
            progress.report({ message: `Importing watch tables...` });
            logger_1.Logger.startOperation(`Import watch tables`);
            const result = await importService.importWatchTablesFromGroup(item.parentId || '', groupName, groupPath, exportPath);
            if (result.success) {
                const deletedCount = result.messages?.filter(m => m.type === 'deleted').length ?? 0;
                const resultWithDeleted = { ...result, deletedCount };
                if (deletedCount > 0) {
                    logger_1.Logger.success(`Watch tables imported: ${deletedCount} deleted, ${result.skippedCount ?? 0} unchanged`);
                }
                else {
                    logger_1.Logger.success(`Watch tables imported: ${result.successCount ?? result.itemCount} tables`);
                }
                ...
                logger_1.Logger.endOperation(`Import watch tables`, true);
                const countMsg = result.successCount ?? result.itemCount;
                vscode.window.showInformationMessage(`Import from TiaPortal: Watch tables imported (${countMsg} tables)`);
            }
            else {
                ...
                logger_1.Logger.endOperation(`Import watch tables`, false);
        ...
        logger_1.Logger.error('Failed to import watch tables', error);
        vscode.window.showErrorMessage(`Import from TiaPortal: ${message}`);
    }
}
//# sourceMappingURL=importWatchTablesCommand.js.map